...
# Example deal data for pre-filling the form
deal_data = {
    "RM-001": {
        "customer": "Alpha Corp",
        "txn_type": "Islamic",
...
form_key = f"deal_form_{selected}"

# Default form state if not already saved
if form_key not in st.session_state:
    st.session_state[form_key] = {
        "txn_type": deal_data[selected]["txn_type"],
        "deal_group": deal_data[selected]["deal_group"],
        "booking_country": deal_data[selected]["booking_country"],
        "transaction_category": deal_data[selected]["transaction_category"]
    }

with st.form(key="deal_edit_form"):
    txn_type = st.selectbox(
        "Transaction Type", ["Islamic", "Conventional"],
        index=0 if st.session_state[form_key]["txn_type"] == "Islamic" else 1
    )
    deal_group = st.selectbox(
        "Deal Group", ["IBG", "CIBG"],
        index=0 if st.session_state[form_key]["deal_group"] == "IBG" else 1
    )
    booking_country = st.selectbox(
        "Booking Country", ["UAE", "KSA", "Qatar"],
        index=["UAE", "KSA", "Qatar"].index(st.session_state[form_key]["booking_country"])
    )
    txn_category = st.text_input(
        "Transaction Category", st.session_state[form_key]["transaction_category"]
    )
    ...
    if submitted:
        ...
        st.success("Changes saved for " + selected)

# Update selected deal in session state
st.session_state.selected_deal_id = selected

# Optionally, display current deal data
st.markdown("### Current Deal Data")
st.json(st.session_state[form_key])